...
func (s *client) V2Handler(ctx *gin.Context) {
	out, err := s.createRequest(ctx, "https://cgr.dev/v2/")
	if err != nil {
		s.logErrorAndAbort(ctx, "Error creating request", err)
		return
	}
...
func (s *client) TokenHandler(ctx *gin.Context) {
	url := s.createTokenURL(ctx)
	out, err := s.createRequest(ctx, url)
	if err != nil {
		s.logErrorAndAbort(ctx, "Error creating request", err)
		return
	}
...
func (s *client) ProxyHandler(ctx *gin.Context) {
	image, ref, err := s.parseImageAndRef(ctx)
	if err != nil {
		s.logErrorAndAbort(ctx, "Error parsing image and ref", err)
		return
	}

	if s.isValidImageAndRef(image, ref) {
		nameWithTag := image + ":" + ref
		r, err := s.imageStorage.FindByNameTag(nameWithTag)
		if err != nil {
			s.logErrorAndAbort(ctx, "Error finding name tag", err)
			return
		}
		if r.HashedIndex != "" {
